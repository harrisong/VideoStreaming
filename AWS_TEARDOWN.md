# Complete AWS Infrastructure Teardown Guide

This guide ensures you can completely remove all AWS resources created for your video streaming service, avoiding any ongoing costs.

## 🚨 Important Warning

**This process will permanently delete ALL data including:**
- Database and all video content
- Container images
- SSL certificates
- Monitoring data
- Backups (unless explicitly preserved)

**Make sure to backup any important data before proceeding!**

## 🔄 Quick Teardown (Recommended)

### Using Terraform (Safest Method)
```bash
# Navigate to your project directory
cd /path/to/VideoStreaming

# Destroy all infrastructure
./terraform-deploy.sh destroy --provider aws --force
...
- ✅ ECS Cluster and all running tasks
...
- ✅ ECR repositories (but not the images - see below)
- ✅ S3 buckets (if empty)
...
```bash
# Stop ECS service (prevents new tasks from starting)
aws ecs update-service \
  --cluster prod-video-streaming \
  --service prod-video-streaming \
  --desired-count 0

# Wait for tasks to stop
aws ecs wait services-stable \
  --cluster prod-video-streaming \
  --services prod-video-streaming
```

### Step 2: Clean Up Container Images
```bash
# List ECR repositories
aws ecr describe-repositories --query 'repositories[?contains(repositoryName, `video-streaming`)].repositoryName' --output table

# Delete all images in backend repository
aws ecr list-images \
  --repository-name prod-video-streaming-backend \
  --query 'imageIds[*]' \
  --output json | \
...
aws ecr list-images \
  --repository-name prod-video-streaming-frontend \
  --query 'imageIds[*]' \
  --output json | \
aws ecr batch-delete-image \
  --repository-name prod-video-streaming-frontend \
...
```bash
# List your video streaming S3 buckets
aws s3 ls | grep video-streaming

# Empty the video bucket (replace with your actual bucket name)
aws s3 rm s3://prod-video-streaming-videos-XXXXXXXX --recursive

# Empty the static assets bucket
aws s3 rm s3://prod-video-streaming-static-XXXXXXXX --recursive

# Delete bucket versions (if versioning was enabled)
aws s3api delete-objects \
  --bucket prod-video-streaming-videos-XXXXXXXX \
...
aws rds create-db-snapshot \
  --db-instance-identifier prod-video-streaming-db \
  --db-snapshot-identifier final-backup-$(date +%Y%m%d)

# Delete RDS instance (skip final snapshot for complete cleanup)
...
### Step 5: Clean Up Networking
```bash
# Delete NAT Gateways (these cost money even when idle)
aws ec2 describe-nat-gateways \
  --filter "Name=tag:Name,Values=*video-streaming*" \
...
aws ec2 describe-addresses \
  --filters "Name=tag:Name,Values=*video-streaming*" \
  --query 'Addresses[].AllocationId' \
  --output text | \
xargs -I {} aws ec2 release-address --allocation-id {}
```

## 🛠️ Automated Teardown Script

Create a comprehensive cleanup script:

<write_to_file>
<path>aws-teardown.sh</path>
<content>
#!/bin/bash
